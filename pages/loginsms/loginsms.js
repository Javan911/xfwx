...
Page({
    ...
    login() {
        ...
        let login = app.globalData.server.wxLoginSms;
        app.console(base, login);
        if (that.data.user.userName.length <= 0) {
            app.console("login","手机号为空");
            app.fail({
                title: "手机号为空",
                icon: "none",
                duration: 1500
    ...
    getCode(){// 获取验证码
        let that = this;
        if (that.data.user.userName.length <= 0 || that.data.user.userName.length != 11) {
            app.console("login", "手机号为空");
            app.fail({
                title: "手机号不正确",
                icon: "none",
        ...
        let base = app.globalData.server.baseUrl;
        let getLoginSms = app.globalData.server.getLoginSms;
        app.httpRequest({
            url: base + getLoginSms,
            data: { phoneNumber: that.data.user.userName},
            callback: function (flag, data) {
                app.console("httpRequest", flag, data);
                if(data.code){
                    app.fail({
                        title: data.msg ? data.msg : "发送成功",
                        icon: "success",
                        duration: 1000
                    });
                }else{
                    app.fail({
                        title: data.msg ? data.msg : "发送失败",
                        icon: "none",
                        duration: 1000
                    });
                }
