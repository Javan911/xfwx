...
Page({
    data: {
        ...
        listLinkman: null,// 报警联系人
    },
    addLinkman(e){// 新增报警联系人
        app.console("新增报警联系人", e);
        wx.navigateTo({
            url: '../addlinkman/addlinkman?groupId=' + e.currentTarget.dataset.data
        });
    },
    editLinkMan(e){// 编辑报警联系人
        app.console("编辑报警联系人", e);
    },
    delLinkMan(e){ // 删除报警联系人
        app.console("删除报警联系人", e);
        let that = this;
        wx.showModal({
            title: '提示',
            content: '您确定删除报警联系人吗',
            success(res) {
                if (res.confirm) {
                    ...
                    let editGroupLinkman = app.globalData.server.editGroupLinkman;
                    that.data.listLinkman.linkman.forEach(function(item, index, arr){
                        if (item.name == e.currentTarget.dataset.name
                            && item.telephone == e.currentTarget.dataset.telephone
                            && item.type == e.currentTarget.dataset.type){
                            arr.splice(item,1);
                        }
                    });
                    let datas = that.data.listLinkman;
                    datas.linkman = JSON.stringify(datas.linkman);
                    app.httpRequest({
                        url: base + editGroupLinkman,
    ...
    listGroupLinkman() { // 加载报警联系人
        ...
        app.console(base, listGroupLinkman, that.data.id);
        wx.showLoading({
            title: '数据加载中...',
        });
        app.httpRequest({
            ...
            callback: function (flag, data) {
                ...
                setTimeout(function () {
                    ...
                    if (flag == 1 && data.FLAG == "SUCCESSED" && data.data.length > 0) {
                        ...
                        datas.linkman = JSON.parse(datas.linkman);
                        that.setData({
                            listLinkman: datas
                        });
                    } else {
        ...
        app.console(that.data.listLinkman);
    },
    onLoad: function (e) {
        this.setData({
            id : e.id
